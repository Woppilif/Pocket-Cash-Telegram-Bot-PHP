...
class Request
{
	...
	{
		$this->apiRequest("sendMessage", array('chat_id' => $this->chat_id, "text" =>  $text));
	}
  ...
  // process incoming message
    ...
    // incoming text message
			...
			if (strpos($text, "/start") === 0) {
				...
				$this->apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Nice to meet you '. $user->getId().' '. $user->getName().'  '. $user->getBalance().' '. $user->getRegDate().' '));
				$user->Hello($chat_id);
